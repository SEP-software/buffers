...
void buffers::updateMemory(const long change2) {
  ...
  std::vector<int> buffers::parsedWindows(const std::vector<int> &nw,
    ...
    for (int i = 0; i < std::min(7, (int)nw.size()); i++) {
      ...
      if (fw[i] + jw[i] * (nw[i] - 1) > ns[i] - 1)
        throw SEPException(std::string("axis[") + std::to_string(i) +
                           std::string("] window out of range f=") +
                           std::to_string(fw[i]) + std::string(" j=") +
                           std::to_string(jw[i]) + std::string(" n=") +
                           std::to_string(nw[i]) + std::string(" ns=") +
                           std::to_string(ns[i]));
  ...
  void buffers::getWindow(const std::vector<int> &nw,
                          const std ::vector<int> &fw,
  ...
  */
  ...
  void buffers::putWindow(const std::vector<int> &nw,
                          const std ::vector<int> &fw,
